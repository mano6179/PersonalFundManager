...
async def zerodha_callback(
    ...
    """Handle callback from Zerodha after login and redirect to frontend popup handler"""
    # Generate session and get access token (and optionally store it)
    auth_response = zerodha_service.generate_session(request_token)
    # Redirect to a dedicated popup handler page in your frontend
    redirect_url = "http://localhost:3000/zerodha-auth-success"
...
async def get_holdings(zerodha_service: ZerodhaService = Depends(get_zerodha_service)):
    ...
    await DatabaseRepository.update_holdings(zerodha_service.user_id, [serialize_model(h) for h in holdings])
    return holdings
...
async def get_positions(zerodha_service: ZerodhaService = Depends(get_zerodha_service)):
    ...
    serialized_positions = {
        pos_type: [serialize_model(p) for p in pos_list]
        for pos_type, pos_list in positions.items()
    }
    await DatabaseRepository.update_positions(zerodha_service.user_id, serialized_positions)
    return positions
